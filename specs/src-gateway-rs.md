...
Pre-flight Planning (>15 words) → Provider (MCP settings write → CLI → MCP cleanup) → Memory Store → Audit Log → Send
```
...
#### `new(provider, channels, memory, auth_config, channel_config, heartbeat_config, scheduler_config, prompts, sandbox_mode, sandbox_prompt) -> Self`
...
**Stage 5: Pre-flight Planning**
- After SILENT suppression check:
- Call `needs_planning(&incoming.text)` to check if the message has more than 15 whitespace-separated words.
- If `needs_planning()` returns true:
  - Call `self.plan_task(&incoming.text)` which sends a dedicated planning prompt to the provider (no system prompt, no history, no MCP servers).
  - The planning response is parsed by `parse_plan_response()`:
  ...
  - If `Some(steps)` is returned:
    - Call `self.execute_steps(incoming, original_task, context, steps, inbox_images)` for autonomous multi-step execution.
    - `execute_steps()` announces the plan to the user, executes each step in a fresh provider call with accumulated context, retries failed steps up to 3 times, sends progress messages after each step, audits the exchange, sends a final summary, and cleans up inbox images.
    - Return (skip normal provider call).
  - If `None` is returned (DIRECT, single-step, or planning failure): fall through to the normal single provider call.
- If `needs_planning()` returns false: fall through to the normal single provider call.
...
**Purpose:** Send a dedicated planning call to the provider to determine if the message requires multi-step execution.

**Parameters:**
- `message: &str` - The user's original message text.

**Returns:** `Option<Vec<String>>` — `Some(steps)` if the planner identifies a multi-step task, `None` if the response is "DIRECT", single-step, or on error.

**Logic:**
1. Build a tiny planning prompt containing the user's message (no system prompt, no conversation history, no MCP servers).
2. Call `provider.complete()` with this minimal context.
3. On success, pass the response text to `parse_plan_response()`.
4. On error, log the error and return `None` (falls through to normal single provider call).
...
**Purpose:** Determine if a message is complex enough to warrant a pre-flight planning call.

**Returns:** `true` if the text has more than 15 whitespace-separated words, `false` otherwise.
...
29. Pre-flight planning: messages with >15 words trigger a dedicated planning call; multi-step plans are executed autonomously with per-step progress, retry (up to 3 attempts), and a final summary.
...
### `test_needs_planning_short_message`

**Type:** Synchronous unit test (`#[test]`)

Verifies that `needs_planning()` returns `false` for short messages with 15 or fewer words.

### `test_needs_planning_long_message`

**Type:** Synchronous unit test (`#[test]`)

Verifies that `needs_planning()` returns `true` for long messages with more than 15 words.
