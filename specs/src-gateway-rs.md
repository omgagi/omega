...
- `prompts`: Externalized prompts and welcome messages, loaded from `~/.omega/prompts/SYSTEM_PROMPT.md` and `~/.omega/prompts/WELCOME.toml` at startup. Falls back to hardcoded defaults if files are missing.
