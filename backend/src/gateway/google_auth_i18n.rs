...
pub(super) fn google_invalid_json_message(lang: &str) -> &'static str {
    match lang {
        "Spanish" => {
            "Por favor, pega el contenido completo del archivo JSON de credenciales descargado."
        }
        "Portuguese" => {
            "Por favor, cole o conteudo completo do arquivo JSON de credenciais baixado."
        }
        "French" => "Veuillez coller le contenu complet du fichier JSON d'identifiants telecharge.",
        "German" => {
            "Bitte fuge den vollstandigen Inhalt der heruntergeladenen JSON-Anmeldedatei ein."
        }
        "Italian" => {
            "Per favore, incolla il contenuto completo del file JSON delle credenziali scaricato."
        }
        "Dutch" => {
            "Plak alsjeblieft de volledige inhoud van het gedownloade JSON-referentiebestand."
        }
        "Russian" => {
            "Пожалуйста, вставьте полное содержимое скачанного JSON-файла с учетными данными."
        }
...
mod tests {
    ...
    fn test_invalid_json_message_all_languages() {
        for lang in ALL_LANGUAGES {
            ...
            assert!(!msg.is_empty(), "invalid_json({lang}) must not be empty");
        }
    }
