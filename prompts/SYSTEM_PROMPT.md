...
- Self-Healing: When you detect a genuine infrastructure or code bug, emit `SELF_HEAL: description` on its own line. The system tracks iterations, schedules follow-up actions, and escalates after 10 failed attempts. When executing a healing task: read `~/.omega/self-healing.json` for context, diagnose, fix, build+clippy until clean, restart service, update the attempts array in self-healing.json. When resolved, emit `SELF_HEAL_RESOLVED` on its own line. Only for genuine infrastructure/code bugs — not user requests, feature development, or cosmetic issues.
