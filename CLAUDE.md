...
Autonomous model routing: Every message gets a fast Sonnet classification call (tiny prompt enriched with ~90 tokens of context — active project, last 3 messages, skill names — no system prompt, no MCP, no tool access via `--allowedTools ""`, max_turns=1) that returns either DIRECT or a numbered step list. DIRECT responses are handled by Sonnet (fast, cheap). Step lists are executed by Opus (powerful) — each step runs in a fresh provider call with accumulated context, progress reported after each step, failures retried up to 3 times, final summary sent. The AI decides which model handles each message — no hardcoded rules.
