...
| `omega-channels` | Messaging platforms (Telegram with voice transcription via Whisper + photo reception, WhatsApp with voice transcription via shared Whisper + image reception + photo sending + group chat + markdown sanitization + send retry with backoff + in-process pairing via `pairing_channels()`) |
| `omega-memory` | SQLite storage, conversation history, audit log, scheduled tasks, task types (reminder/action), limitations (self-introspection), structured user profile formatting, cross-channel user aliases (`user_aliases` table) |
...
- **Phase 4** (in progress): Scheduler (task queue + heartbeat), alternative providers (Ollama + OpenAI + Anthropic API + OpenRouter + Gemini — all implemented with non-streaming HTTP, `to_api_messages()` conversion, factory wiring in `build_provider()` returning `(Box<dyn Provider>, model_fast, model_complex)`), agentic tool-execution loop for all HTTP providers (`tools.rs`: bash/read/write/edit with sandbox enforcement + output truncation, `mcp_client.rs`: JSON-RPC 2.0 over stdio for skill-declared MCP servers, classification safety belt via `allowed_tools = Some(vec![])`), skills system, skill-declared MCP servers (trigger-based activation, dynamic `.claude/settings.local.json` + `--allowedTools` injection for Claude Code; direct `McpClient` stdio connection for HTTP providers), 3-level sandbox (sandbox/rx/rwx workspace isolation + OS-level write enforcement via Seatbelt/Landlock), WhatsApp, cliclack CLI UX, Google Workspace init (via `gog` CLI), OS-aware service management (`omega service install|uninstall|status`), group chat awareness (is_group + SILENT suppression), platform formatting hints, context-aware heartbeat, identity/soul/system prompt split, structured user profile, progressive onboarding (stage-based: 0=intro → 1=/help → 2=/personality → 3=/tasks → 4=/projects → 5=done, tracked via `onboarding_stage` fact, each hint fires once on transition), privacy-focused welcome messages, guided fact-extraction schema, Telegram voice message transcription (OpenAI Whisper), workspace image diff (auto-send provider-created images via send_photo + cleanup), incoming image support (Telegram photo + WhatsApp image → download at channel layer, save to workspace/inbox, inject paths in prompt text for Claude Code Read tool, auto-cleanup after response), autonomous model routing (complexity-aware Sonnet classification — routine actions=DIRECT regardless of quantity, complex work=step list; with active project + history + skills context), auto-resume on max_turns (session-id based, up to 5 attempts), non-blocking gateway (Arc + spawn + message batching per sender), self-introspection (LIMITATION marker → DB store with dedup → Telegram alert → heartbeat auto-add, heartbeat self-audit enrichment), action scheduler (SCHEDULE_ACTION marker → provider-backed autonomous task execution), self-audit (anomaly detection + audit DB query access), self-healing protocol (SELF_HEAL: desc | verification / SELF_HEAL_RESOLVED gateway markers, verification test embedded in follow-up prompts, code-enforced iteration limits + scheduling + escalation via gateway, state in ~/.omega/self-healing.json), quantitative trading engine (omega-quant: standalone CLI binary + library, Kalman filter + HMM regime detection + fractional Kelly sizing + Merton allocation + IBKR TWS API via ibapi + multi-asset support [stocks/forex/crypto] + market scanner + bracket orders [MKT entry + LMT TP + STP SL] + position monitoring + daily P&L queries + close positions + safety guardrails [max positions + P&L cutoff] + TWAP/Immediate execution + circuit breaker + daily limits + crash recovery, 7 CLI subcommands [check/scan/analyze/order/positions/pnl/close], decoupled from gateway — AI invokes via ibkr-quant skill with autonomous trading strategy), conversational command markers (PERSONALITY/FORGET_CONVERSATION/CANCEL_TASK/UPDATE_TASK/PURGE_FACTS — zero-friction equivalents of slash commands, processed via process_markers()), i18n bot command responses (`src/i18n.rs` — all command outputs localized to 8 languages via `t(key, lang)` + format helpers, language resolved from `preferred_language` fact, language-aware onboarding hints), cross-channel user identity (user_aliases table — first channel creates canonical sender_id, subsequent channels alias to it via `resolve_sender_id()` / `create_alias()`, all fact operations share one identity, conversations keep per-channel history), single-bot WhatsApp pairing (gateway uses `pairing_channels()` on the running bot instead of `start_pairing()` which created a conflicting second bot, `as_any()` on Channel trait enables downcasting), task scheduling confirmation (anti-hallucination: `process_markers()` returns `Vec<MarkerResult>` with per-task outcomes [TaskCreated/TaskFailed/TaskParseError/TaskCancelled/TaskCancelFailed/TaskUpdated/TaskUpdateFailed], gateway sends localized confirmation after AI response showing actual DB results for creates/cancels/updates, `descriptions_are_similar()` word-overlap duplicate detection warns about existing similar tasks, multi-extraction via `extract_all_cancel_tasks()` and `extract_all_update_tasks()` processes ALL markers not just the first, system prompt enforces no-pre-confirm rule — `src/task_confirmation.rs`)
