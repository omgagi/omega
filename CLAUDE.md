...
| `omega-sandbox` | OS-level system protection — Seatbelt (macOS), Landlock (Linux) — blocklist approach: blocks writes to dangerous OS dirs + `~/.omega/data/` (memory.db). Always active, no configuration. Also provides `is_write_blocked()` for code-level enforcement in HTTP provider tools. |
...
- **System protection**: Always-on blocklist-based filesystem enforcement via `omega-sandbox`. On macOS (Seatbelt), blocks writes to dangerous OS dirs (`/System`, `/bin`, `/sbin`, `/usr/bin`, `/usr/sbin`, `/usr/lib`, `/usr/libexec`, `/private/etc`, `/Library`) and `~/.omega/data/` (memory.db). On Linux (Landlock), system dirs are read-only, user dirs writable. Code-level `is_write_blocked()` provides additional protection for HTTP provider tool operations. No configuration needed — protection is always active. `~/.omega/stores/` provides a dedicated writable location for domain-specific databases, separate from the protected `~/.omega/data/`.
