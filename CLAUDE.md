...
```
Message → Dispatch (buffer if sender busy, ack) → Auth → Sanitize → Inbox save → Welcome (non-blocking) → Platform Hint → Group Rules → Project hot-reload → Heartbeat awareness → Sandbox constraint → Identity+Soul+System compose → Memory (context) → MCP trigger match → Classify & Route (context-enriched Sonnet classification → DIRECT or step list → model assignment) → [if steps: Opus executes autonomously with progress] → [if direct: Sonnet handles response] → Workspace snapshot → Heads-up → Provider (--model flag + MCP settings write → async CLI + auto-resume on max_turns + status updates → MCP cleanup) → SILENT suppress → Schedule extract → SCHEDULE_ACTION extract → Project activate/deactivate → Lang switch → Heartbeat add/remove → Limitation extract (store + Telegram alert + heartbeat auto-add) → Self-heal extract (state manage + notify + schedule follow-up or escalate) → Self-heal resolved (delete state + notify) → Memory (store) → Audit → Send → Workspace image diff → Inbox cleanup → Drain buffered messages
```
...
- **Heartbeat**: periodic context-aware provider check-in (default 30min), enriched with user facts + recent summaries + open limitations, includes self-audit instruction for autonomous limitation detection, skips when no `~/.omega/HEARTBEAT.md` checklist is configured, suppresses `HEARTBEAT_OK`, alerts otherwise
